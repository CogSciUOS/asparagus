...
def filter_mask_img(img):
    ...
    img = binary_dilation(img, np.ones((60,20))) #sometimes, it cuts the heads off, so mostly vertical dilation
                    ...
                    for box in pieces:
                        ...
                        if( delta_w < 20 or delta_h < 300): # if the segment is too small, it was probably a light reflex
                            ...
                            if(show):
                                ...
                                plt.show()
                            if(save):
                                plt.imsave(os.path.join(target_dir,str(idx)+"_"+str(photo_number)+".jpg"),new_img)
